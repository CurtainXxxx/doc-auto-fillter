...
## 输出说明
系统会自动从对话中提取结构化数据，你只需正常使用工具完成任务即可。会被解析的信息:
- template_type: 模板类型名称
- total_fields: 字段总数
- fields: 字段列表 (name, field_id, type: single/group/datarow)
- facts: 提取的事实 (field_name, value, confidence: high/medium/low, source)
...
### 步骤4：审查汇报
系统会自动提取审查报告结构化数据，你只需正常完成审查并汇报即可。
审查维度会被解析到: passed, fill_rate, filled_count, high_conf_count, medium_conf_count, low_conf_count, missing_fields, logic_errors, review_note

**★ 关键规则：审查不通过时，你只能上报Supervisor，不能自己调DataAgent的工具重新提取数据。数据补充必须由Supervisor调度DataAgent完成。**

**★ 缺失字段请明确列出具体字段名，如：数据申请部门、申请人姓名、工资号**
...
## 输出说明
系统会自动提取文档生成结果，你只需正常调用生成工具即可。会被解析的信息:
- file_path: 生成的文件路径
- template_type: 模板类型
- filled_count: 填充字段数
- success: 是否成功
